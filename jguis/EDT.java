...
*
*Jay Unruh converted this to a library and added the z scaling ratio code so that z distances are in xy pixel units
 */
	
	int w,h,d;
	int current,total;

	public EDT(int w, int h, int d) {
		super();
		this.w = w;
		this.h = h;
		this.d = d;
	...
	public float[][] getEDT(byte[][] mask){
		/*
		 * returns an euclidean distance transform map with a zratio of 1
		 */
		return getEDT(mask,1.0f);
	}
	
	public float[][] getEDT(byte[][] mask,float zratio){
		/*
		 * takes a binary 3D mask and returns a euclidean distance transform map
		 * zratio is the ratio of xy pixel size to z spacing
		 */
		ImageStack stack=jutils.array2stack(mask,w,h);
		float[][] result=new float[d][w*h];
		ImageStack resstack=jutils.array2stack(result,w,h);
		new Z(stack, resstack).compute();
		if(zratio!=1.0f) {
			for(int i=0;i<d;i++){
				float[] temp=(float[])resstack.getPixels(i+1);
				for(int j=0;j<w*h;j++) temp[j]*=zratio;
			}
		}
		new Y(resstack).compute();
		new X(resstack).compute();
		return result;
	}
