...
/*
 * this code was modified from fiji.process3d.SEDT.java
 * 
 ...
 */
public class SEDTcorr {
	ImagePlus image;
	int w, h, d;
	int current, total;
	
	public float[][] compute(Object[] stack,int width,int height,float zratio) {
		ImageStack input=jutils.array2stack(stack,width,height);
		ImagePlus edt=compute(input,zratio);
		Object[] retvals=jutils.stack2array(edt.getStack());
		float[][] retvals2=new float[retvals.length][];
		for(int i=0;i<retvals.length;i++) retvals2[i]=(float[]) retvals[i];
		return retvals2;
	}
