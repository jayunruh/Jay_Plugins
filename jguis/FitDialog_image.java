...
public class FitDialog_image implements DialogListener,NLLSfitinterface_v2{
	public ImagePlus[] imps;
	public float[] datapix,fitpix,residpix;
	...
	/******************************
	 * this class handles the procedures associated with fitting an image, this version uses a grid layout genericdialog
	 * @param imps: an array of imageplus objects showing the data, the fit, and the residuals, autoscaled
	 * @param callclass
	 * @param labels
	 */
	public FitDialog_image(ImagePlus[] imps,NLLSfitinterface_v2 callclass,String[] labels){
		this.imps=imps;
		this.callclass=callclass;
		this.labels=labels;
		this.datapix=(float[])imps[0].getProcessor().getPixels();
		this.fitpix=(float[])imps[1].getProcessor().getPixels();
		this.residpix=(float[])imps[2].getProcessor().getPixels();
	...
	public void run_fit(double[] params,float[] weights,double[][] constraints,int[] fixes){
		...
		while(showoptions(params,fixes)){
			...
			float[] fit=fitclass.fitdata(params,fixes,constraints,datapix,weights,stats,true);
			float[] resid=fitclass.calculate_resid_fit(algutils.convert_arr_double(fit),datapix);
			c2=(float)stats[1];
			//if(pw!=null) pw.updateSeries(fit,1,true);
			System.arraycopy(fit,0,fitpix,0,fit.length);
			System.arraycopy(resid,0,residpix,0,resid.length);
			imps[1].updateAndDraw(); imps[1].resetDisplayRange();
			imps[2].updateAndDraw(); imps[2].resetDisplayRange();
	...
	public boolean get_errors(double[] params,int[] fixes){
		...
		if(methodindex==0){
			...
			double[][] c2plot=erclass.geterrors(params,fixes,constraints,datapix,weights,flim,truespacing,errindex);
			...
			double[][] errors=erclass.geterrors(params,fixes,constraints,datapix,weights,ntrials);
			sb=new StringBuffer();
			sb.append("StDev\t");
	...
	public boolean dialogItemChanged(GenericDialog gd,AWTEvent e){
		...
		double[] stats=new double[2];
		float[] fit=fitclass.fitdata(params,fixes,null,datapix,weights,stats,true);
		float[] resid=fitclass.calculate_resid_fit(algutils.convert_arr_double(fit),datapix);
		//if(pw!=null) pw.updateSeries(fit,1,true);
		System.arraycopy(fit,0,fitpix,0,fit.length);
		System.arraycopy(resid,0,residpix,0,resid.length);
