...
public class PDFPlotter extends Plotter{
	public PDFGraphics2D vg2;
	public boolean antialias;

	public PDFPlotter(int width,int height,OutputStream os){
		this.width=width;
		this.height=height;
		shapesize=8;
		try{
			vg2=new PDFGraphics2D(os,new Dimension(width,height));
	...
	public PDFPlotter(int width,int height,String path){
		this.width=height;
		this.height=height;
		shapesize=8;
		try{
			File file=new File(path);
			vg2=new PDFGraphics2D(file,new Dimension(width,height));
			vg2.setDeviceIndependent(true);
			vg2.startExport();
		}catch(Throwable e){
			IJ.log(e.toString());
