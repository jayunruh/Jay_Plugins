...
public class matrixsolve{
	...
	public double[][] gjinv2(double[][] A,int size){
		...
		if(size==2){ //and the 2 x 2 solution
			double a1=A[0][0], b1=A[1][0], c1=A[0][1], d1=A[1][1];
			double factor=1.0/(a1*d1-b1*c1);
			inv[0][0]=factor*d1; inv[0][1]=-factor*b1;
			inv[1][0]=-factor*c1; inv[1][1]=factor*a1;
			return inv;
		}
		if(size==3){ //and the 3 x 3 solution
			double factor=A[0][0]*A[1][1]*A[2][2]-A[0][0]*A[1][2]*A[2][1]-A[0][1]*A[1][0]*A[2][2]+
					A[0][1]*A[1][2]*A[2][0]+A[0][2]*A[1][0]*A[2][1]-A[0][2]*A[1][1]*A[2][0];
			factor=1.0/factor;
			inv[0][0]=factor*(A[1][1]*A[2][2]-A[2][1]*A[1][2]); inv[0][1]=factor*(A[0][2]*A[2][1]-A[2][2]*A[0][1]); inv[0][2]=factor*(A[0][1]*A[1][2]-A[1][1]*A[0][2]);
			inv[1][0]=factor*(A[1][2]*A[2][0]-A[2][2]*A[1][0]); inv[1][1]=factor*(A[0][0]*A[2][2]-A[2][0]*A[0][2]); inv[1][2]=factor*(A[0][2]*A[1][0]-A[1][2]*A[0][0]);
			inv[2][0]=factor*(A[1][0]*A[2][1]-A[2][0]*A[1][1]); inv[2][1]=factor*(A[0][1]*A[2][0]-A[2][1]*A[0][0]); inv[2][2]=factor*(A[0][0]*A[1][1]-A[1][0]*A[0][1]);
			return inv;
		}
		// copy the A matrix to the left side of the gj matrix
		for(i=0;i<size;i++){
