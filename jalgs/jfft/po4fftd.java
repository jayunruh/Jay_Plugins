...
public class po4fftd{
	public int[] bitorder;
	public double[] cosvals;
	public int length;

	/*
	 * here is a basic complex power of 4 complex fft once the po4fft class has
	 * been created it can be used again for other ffts of the same size data
	 * must be power of 2 length Copyright Jay Unruh Stowers Institute for
	 * Medical Research 4/25/08 note that bit reversal is made optional to allow
	 * for the calling class to do the bit reversal ahead of time. This can save
	 * extra copying If the bitreversal is done ahead of time, it is wise to
	 * initialize this class with the current bitorder array to save memory this
	 * same logic applies to all of the fft classes
	 */

	public po4fftd(int size){
		length=size;
		initrevbits();
		initcosvals();
	}

	public po4fftd(int size,int[] bitorder1,double[] cosvals1){
		length=size;
		bitorder=bitorder1;
		cosvals=cosvals1;
	}

	public po4fftd(int size,double[] cosvals1){
		length=size;
		cosvals=cosvals1;
		initrevbits();
	}
			...
			while(increment<length){
				for(int i=0;i<length;i+=(increment+increment)){
					for(int k=0;k<increment;k++){
						...
						double tempcos=cosvals[trigindex];
						double tempsin;
						if(trigindex<ld4){
						...
						double flt1=tempcos*r[temp2]+tempsin*im[temp2];
						double flt2=tempcos*im[temp2]-tempsin*r[temp2];
