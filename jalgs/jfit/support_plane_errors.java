...
public class support_plane_errors implements NLLSfitinterface{
	...
	public support_plane_errors(NLLSfitinterface fitinterface,double toler1,int maxiter1,boolean global1,double lambda1){
		fitclass=fitinterface;
		toler=toler1;
		maxiter=maxiter1;
		global=global1;
		if(global){
			fitfunctionclass=new NLLSglobalfit(this,toler,maxiter,lambda1);
		}else{
			fitfunctionclass=new NLLSfit(this,toler,maxiter,lambda1);
		}
	}

	public support_plane_errors(NLLSfitinterface fitinterface,double toler1,int maxiter1,boolean global1){
		fitclass=fitinterface;
		toler=toler1;
		maxiter=maxiter1;
		global=global1;
		if(global){
			fitfunctionclass=new NLLSglobalfit(this,toler,maxiter);
		}else{
			fitfunctionclass=new NLLSfit(this,toler,maxiter);
		}
	}

	public support_plane_errors(NLLSfitinterface fitinterface,int maxiter1,boolean global1){
		fitclass=fitinterface;
		maxiter=maxiter1;
		toler=0.0001;
		global=global1;
	...
	public support_plane_errors(NLLSfitinterface fitinterface,double toler1,boolean global1){
		fitclass=fitinterface;
		toler=toler1;
		maxiter=50;
		global=global1;
	...
	public support_plane_errors(NLLSfitinterface fitinterface,boolean global1){
		fitclass=fitinterface;
		toler=0.0001;
		maxiter=50;
		global=global1;
	...
	public double[][] geterrors(double[] params,int[] fixes,double[][] constraints,float[] data,float[] weights,double flim,double spacing,int errindex){
		...
		((NLLSfit)fitfunctionclass).maxiter=0;
		int oldfix=fixes[errindex];
		fixes[errindex]=1;
		double[] stats=new double[2];
		((NLLSfit)fitfunctionclass).fitdata(params,fixes,constraints,data,weights,stats,false);
		((NLLSfit)fitfunctionclass).maxiter=maxiter;
	...
	public double[][] geterrorsglobal(double[][] params,int[][] vflmatrix,String[][] formulas,String[] paramsnames,double[][][] constraints,float[][] data,float[][] weights,double flim,
		...
		((NLLSglobalfit)fitfunctionclass).maxiter=0;
		int oldfix=vflmatrix[errindex[1]][errindex[0]];
		vflmatrix[errindex[1]][errindex[0]]=1;
		double[] stats=new double[2];
		((NLLSglobalfit)fitfunctionclass).fitdata(params,vflmatrix,formulas,paramsnames,constraints,data,weights,stats,new double[params.length],false);
		((NLLSglobalfit)fitfunctionclass).maxiter=maxiter;
		double minc2=stats[1];
		double[][] tempc2plot=new double[2][10000];
		tempc2plot[0][0]=thisparam;
		tempc2plot[1][0]=minc2;
	...
	public double fitfunc(double[] params,int indvar){
		return fitclass.fitfunc(params,indvar);
	}
