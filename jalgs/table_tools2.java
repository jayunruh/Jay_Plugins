...
public class table_tools2{
	//this class is for table tools that don't reference ImageJ
	//many are copies from jguis.table_tools
	...
	public static List<List<String>> getTableFromFile(File infile,String delim,boolean noconsec){
		try{
			...
			List<List<String>> listtable=table_tools2.table2listtable(lines,delim,noconsec);
			lines=null;
			b.close();
			return listtable;
		}
	...
	public static boolean writeTableToFile(String path,List<List<String>> listtable,int delim){
		//this version assumes the column labels are in the first row
		String[] collabels=list2stringarray(listtable.get(0));
		listtable.remove(0);
		return writeTableToFile(path,collabels,listtable,delim);
	}
	...
	public static List<String> get_cell_stat(List<List<String>> list,int cellcolumn,String cellid,String stat,boolean addct,float[] options){
		...
		List<String> stats=table_tools2.get_table_stat(celltable,stat,options);
